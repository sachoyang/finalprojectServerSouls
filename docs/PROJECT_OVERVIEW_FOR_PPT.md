...
### 4-4. 두 개의 서버 채널 — 왜 나눴나 (첫 청중용 설명)

| 구분 | ☁ Photon (실시간 채널) | 🖥 웹 API (Apache·PHP·MySQL) |
|---|---|---|
| 목적 | 전투 순간을 서로 똑같이 보이게 | 영구 저장·조회가 필요한 것 |
| 데이터 | 위치·애니메이션·보스 상태·데미지 | 계정·스킬 해금·랭킹·크래시 로그 |
| 방식 | **계속 연결**된 양방향 통신 | **필요할 때 1회** 요청→응답(HTTP) |
| 빈도 | 초당 수십 번(쉼 없이) | 로그인·보상·클리어 순간에만 |
| 특징 | 호스트 권한, 저장 안 함(휘발) | DB에 남아 다음 접속에도 유지 |

> 비유: **Photon은 전화 통화**(계속 연결·실시간), **웹 API는 편지**(필요할 때 보내고 답장 받아 DB에 보관).

### 4-5. 실제 서버 API (`soulrush_api`, PHP) & 통신 흐름

**엔드포인트 (게임 ↔ 서버)**
- **인증·세션**: `register.php` · `login.php` · `check_session.php`(15초 하트비트, 중복 로그인 차단) · `check_admin.php`
- **스킬**: `get_abilities.php`(수치 카탈로그 조회) · `update_skills.php`(해금 비트마스크 저장) · `upload_ability.php`(에디터 업로드)
- **랭킹·크래시**: `submit_ranking.php` / `get_rankings.php` · `crash_report.php`
- **관리용 웹(브라우저)**: `admin_hub` · `crash_viewer` · `ranking_viewer` · `log_viewer` — 같은 MySQL 조회
- **DB 테이블**: `users` · `abilities`(공통1+타입별6=7) · `team_rankings` · `crash_reports`

**접속~클리어 통신 흐름**
1. **서버 자동 탐지** — 같은 랜이면 LAN IP, 아니면 공인 IP(포트 8080) `[BackendManager]`
2. **로그인** `login.php` → 세션 토큰 + 해금 비트마스크 수신
3. **세션 유지** 15초마다 `check_session.php` → 중복 접속 차단
4. **스킬 최신화** `get_abilities.php` → 서버 밸런스 수치 반영
5. **클리어** `submit_ranking.php` → 팀 기록 등록
